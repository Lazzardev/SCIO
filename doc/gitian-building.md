...
*Setup instructions for a gitian build of SCIO using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the SCIO
...
Clone the git repositories for scio and gitian and then checkout the scio version that you want to build.
...
Follow the instructions in [doc/release-process.md](release-process.md) in the scio repository
...
To build SCIO (for Linux, OSX and Windows) just follow the steps under 'perform
gitian builds' in [doc/release-process.md](release-process.md) in the scio repository.
...
```bash
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/scio/.git/
...
./bin/gbuild --commit scio=${COMMIT} --url scio=${URL} ../SCIO/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit scio=${COMMIT} --url scio=${URL} ../SCIO/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit scio=${COMMIT} --url scio=${URL} ../SCIO/contrib/gitian-descriptors/gitian-osx.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/scio-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/scio-build.assert
    gpg --detach-sign ${VERSION}-osx/${SIGNER}/scio-build.assert
```
