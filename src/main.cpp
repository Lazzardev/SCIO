...
#if defined(NDEBUG)
#error "SCIO cannot be compiled without assertions."
#endif
...
unsigned int nStakeMinAge = 4 * 60 * 60; //stake age 6h
...

    {
        ...
        if(tx.IsZerocoinSpend()){
            ...
            if (IsTransactionInChain(tx.GetHash(), nHeightTx))
                return state.Invalid(error("AcceptToMemoryPool : zSCIO spend tx %s already in block %d", tx.GetHash().GetHex(), nHeightTx),
            ...
            for (const CTxIn& txIn : tx.vin) {
                ...
                if (IsSerialInBlockchain(spend.getCoinSerialNumber(), nHeightTx))
                    return state.Invalid(error("%s : zSCIO spend with serial %s is already in block %d\n",
                ...
                if (!spend.HasValidSerial(Params().Zerocoin_Params()))
                    return state.Invalid(error("%s : zSCIO spend with serial %s from tx %s is not in valid range\n",
...
int64_t GetBlockValue(int nHeight)
{
    ...
    if (nHeight == 0) {
  	...
  	} else if (nHeight <= 275 && nHeight > 0) {
  		nSubsidy = 1 * COIN;
  	} else if (nHeight <= 8000 && nHeight > 275) {
  		nSubsidy = 1 * COIN;
  	} else if (nHeight <= 16000 && nHeight > 8000) {
  		nSubsidy = 4 * COIN;
  	} else if (nHeight <= 32000 && nHeight > 16000) {
  		nSubsidy = 9 * COIN;
  	} else if (nHeight <= 64000 && nHeight > 32000) {
  		nSubsidy = 13 * COIN;
  	} else if (nHeight <= 128000 && nHeight > 64000) {
  		nSubsidy = 17 * COIN;
  	} else if (nHeight <= 256000 && nHeight > 128000) {
  		nSubsidy = 21 * COIN;
  	} else if (nHeight <= 512000 && nHeight > 256000) {
  		nSubsidy = 25 * COIN;
  	} else if (nHeight <= 1024000 && nHeight > 512000) {
  		nSubsidy = 29 * COIN;
  	} else if (nHeight > 1024000) {
  		nSubsidy = 33 * COIN;
  	} else {
  		nSubsidy = 33 * COIN;
  	}
...
int64_t GetMasternodePayment(int nHeight, int64_t blockValue, int nMasternodeCount)
{
	...
	if (nHeight == 0) {
	      ret = blockValue  / 100 * 0;
	} else if (nHeight <= 32000 && nHeight > 275) {
		  ret = blockValue  / 100 * 90;
	} else if (nHeight <= 128000 && nHeight > 32000) {
		  ret = blockValue  / 100 * 80;
	} else if (nHeight > 128000) {
		if(nHeight >= GetForkHeight())
		  ret = blockValue  / 100 * 80; // 5% devfee
		else
		  ret = blockValue  / 100 * 80;
	} else {
  		  ret = blockValue  / 100 * 0;
...
bool DisconnectBlock(CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool* pfClean)
{
    ...
    for (int i = block.vtx.size() - 1; i >= 0; i--) {
         ...
         * note we only undo zerocoin databasing in the following statement, value to and from SCIO
...
{
    RenameThread("scio-scriptch");
    scriptcheckqueue.Thread();
}

void RecalculateZSCIOMinted()
{
...
void RecalculateZSCIOSpent()
{
    ...
    while (true) {
        ...
        //Remove spends from zSCIO supply
...
bool RecalculateSCIOSupply(int nHeightStart)
{
                ...
                if (zerocoinDB->ReadCoinSpend(spend.getCoinSerialNumber(), hashTxFromDB)) {
                    if(IsSerialInBlockchain(spend.getCoinSerialNumber(), nHeightTxSpend)) {
                        if(!fVerifyingBlocks || (fVerifyingBlocks && pindex->nHeight > nHeightTxSpend))
                            return state.DoS(100, error("%s : zSCIO with serial %s is already in the block %d\n",
...
//    LogPrintf("XX69----------> ConnectBlock(): nValueOut: %s, nValueIn: %s, nFees: %s, nMint: %s zSCIOSpent: %s\n",
...
void static UpdateTip(CBlockIndex* pindexNew)
{
    ...
    // If turned on AutoZeromint will automatically convert SCIO to zSCIO
...
bool CheckBlock(const CBlock& block, CValidationState& state, bool fCheckPOW, bool fCheckMerkleRoot, bool fCheckSig)
{
    ...
    for (const CTransaction& tx : block.vtx) {
        ...
        // double check that there are no double spent zSCIO spends in this block
        if (tx.IsZerocoinSpend()) {
            for (const CTxIn txIn : tx.vin) {
                if (txIn.scriptSig.IsZerocoinSpend()) {
                    libzerocoin::CoinSpend spend = TxInToZerocoinSpend(txIn);
                    if (count(vBlockSerials.begin(), vBlockSerials.end(), spend.getCoinSerialNumber()))
                        return state.DoS(100, error("%s : Double spending of zSCIO serial %s in block\n Block: %s",
...
bool ProcessNewBlock(CValidationState& state, CNode* pfrom, CBlock* pblock, CDiskBlockPos* dbp)
{
    ...
    if (nMints || nSpends)
        LogPrintf("%s : block contains %d zSCIO mints and %d zSCIO spends\n", __func__, nMints, nSpends);
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv, int64_t nTimeReceived)
{
    ...
    if (strCommand == "version") {
        ...
        // SCIO: We use certain sporks during IBD, so check to see if they are
