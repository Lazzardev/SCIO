                                                          ...
                                                          currentBalance(-1)
{
    ...
    // "Spending 999999 zSCIO ought to be enough for anybody." - Bill Gates, 2017
    ui->zSCIOpayAmount->setValidator( new QDoubleValidator(0.0, 21000000.0, 20, this) );
    ...
    ui->labelzSCIOSyncStatus->setText("(" + tr("out of sync") + ")");
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        ui->pushButtonMintzSCIO->setEnabled(false);
        ui->pushButtonMintzSCIO->setToolTip(tr("zSCIO is currently disabled due to maintenance."));

        ui->pushButtonSpendzSCIO->setEnabled(false);
        ui->pushButtonSpendzSCIO->setToolTip(tr("zSCIO is currently disabled due to maintenance."));
    }
}
...
void PrivacyDialog::on_addressBookButton_clicked()
{
    ...
    if (dlg.exec()) {
        ...
        ui->zSCIOpayAmount->setFocus();
    }
}

void PrivacyDialog::on_pushButtonMintzSCIO_clicked()
{
    if (!walletModel || !walletModel->getOptionsModel())
        return;
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zSCIO is currently undergoing maintenance."), QMessageBox::Ok,
                                 QMessageBox::Ok);
        return;
    }
    ...
    ui->TEMintStatus->setPlainText(tr("Minting ") + ui->labelMintAmountValue->text() + " zSCIO...");
    ...
    QString strStatsHeader = tr("Successfully minted ") + ui->labelMintAmountValue->text() + tr(" zSCIO in ") +
...
void PrivacyDialog::on_pushButtonSpendzSCIO_clicked()
{
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zSCIO is currently undergoing maintenance."), QMessageBox::Ok, QMessageBox::Ok);
        return;
    }
    ...
    if (encStatus == walletModel->Locked || encStatus == walletModel->UnlockedForAnonymizationOnly) {
        ...
        // Wallet is unlocked now, sedn zSCIO
        sendzSCIO();
        return;
    }
    // Wallet already unlocked or not encrypted at all, send zSCIO
    sendzSCIO();
}

void PrivacyDialog::on_pushButtonZSCIOControl_clicked()
{
    ZSCIOControlDialog* zSCIOControl = new ZSCIOControlDialog(this);
    zSCIOControl->setModel(walletModel);
    zSCIOControl->exec();
}

void PrivacyDialog::setZSCIOControlLabels(int64_t nAmount, int nQuantity)
{
    ui->labelzSCIOSelected_int->setText(QString::number(nAmount));
...
void PrivacyDialog::sendzSCIO()
{
    ...
    if(ui->payTo->text().isEmpty()){
        ...
        if (!address.IsValid()) {
            QMessageBox::warning(this, tr("Spend Zerocoin"), tr("Invalid SCIO Address"), QMessageBox::Ok, QMessageBox::Ok);
            ui->payTo->setFocus();
            return;
        }
    }
    ...
    double dAmount = ui->zSCIOpayAmount->text().toDouble();
    ...
    // Warn for additional fees if amount is not an integer and change as zSCIO is requested
    ...
    if(!fWholeNumber && fMintChange){
        ...
        strFeeWarning += QString::number(dzFee, 'f', 8) + " SCIO </b>will be added to the standard transaction fees!<br />";
    ...
    QString strAmount = "<b>" + QString::number(dAmount, 'f', 8) + " zSCIO</b>";
    ...
    // use mints from zSCIO selector if applicable
    vector<CZerocoinMint> vMintsSelected;
    if (!ZSCIOControlDialog::listSelectedMints.empty()) {
        vMintsSelected = ZSCIOControlDialog::GetSelectedMints();
    }
    ...
    // Clear zscio selector in case it was used
    ZSCIOControlDialog::listSelectedMints.clear();
    ...
    for (CZerocoinSpend spend : receipt.GetSpends()) {
        strStats += tr("zSCIO Spend #: ") + QString::number(nCount) + ", ";
    ...
    for (const CTxOut& txout: wtxNew.vout) {
        strStats += tr("value out: ") + FormatMoney(txout.nValue).c_str() + " SCIO, ";
        nValueOut += txout.nValue;

        strStats += tr("address: ");
        CTxDestination dest;
        if(txout.scriptPubKey.IsZerocoinMint())
            strStats += tr("zSCIO Mint");
                               ...
                               const CAmount& watchOnlyBalance, const CAmount& watchUnconfBalance, const CAmount& watchImmatureBalance)
{
    ...
    for (const auto& denom : libzerocoin::zerocoinDenomList) {
        ...
        strDenomStats = strUnconfirmed + QString::number(mapDenomBalances.at(denom)) + " x " +
                        ...
                        QString::number(nSumPerCoin) + " zSCIO </b>";
    ...
    ui->labelzAvailableAmount->setText(QString::number(zerocoinBalance/COIN) + QString(" zSCIO "));
    ui->labelzAvailableAmount_2->setText(QString::number(matureZerocoinBalance/COIN) + QString(" zSCIO "));
    ui->labelzSCIOAmountValue->setText(BitcoinUnits::floorHtmlWithUnit(nDisplayUnit, balance - immatureBalance - nLockedBalance, false, BitcoinUnits::separatorAlways));
}
...
{
    ui->labelzSCIOSyncStatus->setVisible(fShow);
}
