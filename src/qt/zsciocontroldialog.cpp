...
#include "ui_zsciocontroldialog.h"
...
std::list<std::string> ZSCIOControlDialog::listSelectedMints;
std::list<CZerocoinMint> ZSCIOControlDialog::listMints;

ZSCIOControlDialog::ZSCIOControlDialog(QWidget *parent) :
    QDialog(parent),
    ui(new Ui::ZSCIOControlDialog),
    model(0)
{
    ui->setupUi(this);
    listMints.clear();
...
ZSCIOControlDialog::~ZSCIOControlDialog()
{
    delete ui;
}

void ZSCIOControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZSCIOControlDialog::updateList()
{
...
void ZSCIOControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZSCIOControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CZerocoinMint mint : listMints) {
    ...
    ui->labelZSCIO_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(listSelectedMints.size()));

    //update PrivacyDialog labels
    privacyDialog->setZSCIOControlLabels(nAmount, listSelectedMints.size());
}

std::vector<CZerocoinMint> ZSCIOControlDialog::GetSelectedMints()
{
...
void ZSCIOControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
